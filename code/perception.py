...
def perception_step(Rover):
    ...
    # Fidelity Optimization: transforms are only valid when roll and pitch angles are near 0
    #Threshold is within 1 degrees
    pitch = float(Rover.pitch)
    roll = float(Rover.roll)
    pitch_within_threshold = (pitch < 1.0 and pitch >= 0.0) or (pitch < 360 and pitch >= 359) 
    roll_within_threshold = (roll < 1.0 and roll >= 0.0) or (roll < 360 and roll >= 359) 
    if pitch_within_threshold and roll_within_threshold :
        ...
        for image_type in range(0, 3): 
            ...
            Rover.vision_image[:,:,image_type] = np.copy(threshed)
